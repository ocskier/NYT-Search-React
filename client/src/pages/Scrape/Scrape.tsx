...
interface State {
  ...
  articlesAdded: Array<string>
}

class Scrape extends Component<State> {
  state = {
    articles: [],
    articlesAdded: []
  };

  scrapeArticles = (e: React.MouseEvent) => {
    e.preventDefault();
    API.getScrape()
      .then(res => {
        console.log(res);
        this.setState({ articles: res.data }, () => console.log(this.state));
      })
      .catch(err => {
        console.log(err);
      });
  ...
  addArticle = (e: React.BaseSyntheticEvent) => {
    e.preventDefault();
    const el = e.target;

    API.makeArticle(this.state.articles[el.id])
      .then(res => {
                ...
                >
                  ...
                  <footer className="w3-container w3-blue">
                    <h5>
                      ...
                      >
                        {this.checkSaved(index) ? "Added" : "Save"}
                      </button>
                    </h5>
                  </footer>
                </Col>
